...
def _hash_file_group(root: Path, paths: list[Path]) -> dict[str, Any]:
    ...
    for path in sorted(paths):
        ...
        files[resolved.relative_to(resolved_root).as_posix()] = _sha256(resolved)
    if not files:
...
def _validate_visible_manifest(case_root: Path) -> str:
    ...
    actual_names = {path.relative_to(case_root).as_posix() for path in actual_paths}
...
def _preflight(
    ...
    *,
    ...
    for view, directory_name in CASE_DIRECTORIES.items():
        ...
        visible_file_hashes = {
            path.relative_to(case_root).as_posix(): _sha256(path)
