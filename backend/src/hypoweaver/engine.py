...
from .models import (
    ...
    ModelCallContext,
    ModelSpec,
    MIN_FULL_MANUSCRIPT_SECTION_CHARS,
    PlannedStep,
    PolicyDesignSpec,
    PromptContent,
    ProbeCheck,
...
def _is_reviewer_issue_blocking_design(
    ...
    if method_family == "policy_causal":
        ...
        if issue.threat_id in _POLICY_DID_DELEGATED_THREATS:
            return False
...
def _is_group1_sign_switch_package(package: ResearchPackage) -> bool:
    provenance = package.upstream_provenance
    visible_fields = {variable.name for variable in package.variables}
    return (
        provenance is not None
        and provenance.source_system == "group1-discovery"
        and _GROUP1_SIGN_SWITCH_FIELDS.issubset(visible_fields)
    )


def _normalize_group1_sign_switch_route(
    route: MethodRoute,
    package: ResearchPackage,
) -> MethodRoute:
    """Keep the frozen Group 1 causal estimand from degrading to association."""

    if not _is_group1_sign_switch_package(package):
        return route
    required_assumptions = list(
        dict.fromkeys(
            [
                "Cohort-specific parallel trends conditional on the frozen pre-treatment capacity measure.",
                "No anticipation before each region's treatment cohort.",
                "Not-yet-treated or never-treated observations provide valid cohort-time comparisons.",
                "The digital/fintech capacity moderator is measured strictly before treatment.",
                "Firm-to-region and region-to-policy-cohort joins are auditable and time consistent.",
                *route.required_assumptions,
            ]
        )
    )
    missing_information = list(
        dict.fromkeys(
            [
                *(package.intake_readiness.blockers if package.intake_readiness else []),
    ...
    return route.model_copy(
        update={
            ...
            "route_reason": [
                "The verified Group 1 handoff freezes a staggered policy-cohort sign-switch estimand; a generic panel association would change the approved scientific boundary."
            ],
            "required_assumptions": required_assumptions,
            "testable_assumptions": required_assumptions[:4],
            "untestable_assumptions": required_assumptions[4:],
            "alternative_routes": [],
...
def _normalize_group1_sign_switch_candidate_plan(
    ...
    """Freeze an honest design-only staggered-DID/DDD contract for Group 1.

    The existing ``policy-did-v2`` executor implements a common-date binary-group
    DID.  It must not be presented as an implementation of the upstream
    cohort-robust, continuous-moderator estimand.  This normalizer therefore
    binds the requested scientific design while marking every estimative step as
    non-executable until the dedicated adapter and required datasets exist.
    """

    if not _is_group1_sign_switch_package(package):
        return plan

    variables = {variable.name: variable for variable in package.variables}
    outcomes = [
        name
        for name in ("greenwashing_gap", "firm_emission_intensity")
        if name in variables
    ]
    mechanisms = [
        name
        for name in ("rd_expenditure", "green_patent_count", "epie")
        if name in variables
    ]
    capacity = "prepolicy_digital_fintech_capacity"
    exposure = "gfripz_exposure"
    cohort = "treatment_cohort_year"
    entity = "firm_id"
    time = "year"
    region = "region_id"
    ...
    not_executable_reason = (
        ...
        "cohort-robust staggered-DID continuous-capacity DDD executor adapter."
    )
    strategy_contracts = {
        "direct_baseline": {
            "focus": "minimal_primary_estimand",
            "capacity_function": "linear_with_preregistered_zero_crossing",
            "support_rule": "full_preregistered_common_support",
        },
        "identification_first": {
            "focus": "cohort_overlap_and_pretrend_first",
            "capacity_function": "linear_with_preregistered_zero_crossing",
            "support_rule": "cohort_overlap_trimmed_before_estimation",
        },
        "measurement_robustness": {
            "focus": "paired_outcome_measurement_and_missingness",
            "capacity_function": "linear_primary_with_preregistered_spline_sensitivity",
            "support_rule": "paired_outcome_common_support",
        },
    }
    strategy_contract = strategy_contracts[strategy]
    design_contract = {
        "design_type": "cohort_robust_staggered_did_continuous_capacity_ddd",
        "cohort_field": cohort,
        "time_field": time,
        "entity_field": entity,
        "assignment_region_field": region,
        "exposure_field": exposure,
        "moderator_field": capacity,
        "moderator_timing": "strictly_pre_treatment_cohort",
        "comparison_group": "not_yet_treated_or_never_treated",
        "aggregation": "cohort_by_event_time_then_prespecified_weighting",
        "candidate_strategy": strategy_contract,
    }
    claim_ids = [f"claim-{item.hypothesis_id}" for item in package.hypotheses]

    estimands = [
        PlannedStep(
            step_id="estimand-cohort-event-capacity-slope",
            name="Cohort/event-time policy effect over baseline capacity",
            rationale=(
                "Estimate cohort-robust policy effects and their prespecified slope over "
                "strictly pre-treatment digital/fintech capacity."
            ),
            required_data_fields=[entity, time, region, cohort, exposure, capacity],
            parameters=design_contract,
            target_claim_ids=claim_ids,
            required_for_admission=True,
            not_executable_reason=not_executable_reason,
        ),
        PlannedStep(
            step_id="estimand-preregistered-zero-crossing",
            name="Preregistered sign-switch zero crossing",
            rationale=(
                "Locate the capacity value at which the marginal policy effect crosses zero "
                "without selecting a threshold after observing outcomes."
            ),
            required_data_fields=[cohort, exposure, capacity, *outcomes],
            parameters={
                **design_contract,
                "zero_crossing_rule": "freeze_support_and_function_before_outcomes",
            },
            target_claim_ids=claim_ids,
            required_for_admission=True,
            not_executable_reason=not_executable_reason,
        ...
        PlannedStep(
            step_id="estimand-paired-real-reported-alignment",
            name="Paired greenwashing and direct-emissions alignment",
            rationale=(
                "Estimate both outcomes under the same cohort, support and moderator definition."
            ),
            required_data_fields=[entity, time, cohort, exposure, capacity, *outcomes],
            parameters={**design_contract, "paired_outcomes": outcomes},
            target_claim_ids=claim_ids,
            required_for_admission=True,
            not_executable_reason=not_executable_reason,
        ),
    ...
    sample_rules = [
        PlannedStep(
            step_id="sample-freeze-firm-cohort-panel",
            name="Freeze firm-year cohort panel before outcomes",
            rationale=(
                "Bind each firm-year to one auditable region and first-treatment cohort; "
                "retain never-treated and not-yet-treated comparison observations."
            ),
            required_data_fields=[entity, time, region, cohort],
            parameters={
                "primary_key": [entity, time],
                "cohort_key": [region, cohort],
                "comparison_group": design_contract["comparison_group"],
            },
            not_executable_reason=not_executable_reason,
        )
    ]
    variable_construction = [
        PlannedStep(
            step_id="construct-prepolicy-capacity",
            name="Freeze pre-treatment capacity",
            rationale=(
                "Construct capacity only from observations preceding each region's treatment "
                "cohort; contemporaneous or post-treatment capacity is prohibited."
            ),
            required_data_fields=[region, time, cohort, capacity],
            parameters={
                "moderator": capacity,
                "allowed_window": "strictly_before_treatment_cohort",
            },
            not_executable_reason=not_executable_reason,
        ),
        PlannedStep(
            step_id="construct-paired-outcomes",
            name="Freeze paired disclosure and direct-carbon outcomes",
            rationale=(
                "Keep the greenwashing gap and direct firm carbon intensity distinct and "
                "apply defensible, preregistered denominators."
            ),
            required_data_fields=outcomes,
            parameters={"paired_outcomes": outcomes, "silent_substitution": False},
            not_executable_reason=not_executable_reason,
        ),
    ]
    baseline_models = [
        ModelSpec(
            step_id=f"model-staggered-ddd-{outcome}",
            name=f"Cohort-robust staggered DID/DDD for {outcome}",
            rationale=(
                "Primary outcome-specific model for the frozen conditional sign-switch claim."
            ),
            ...
            estimator="cohort-robust staggered-adoption DID with continuous baseline-capacity DDD",
            formula=(
                f"{outcome} ~ cohort_event_time({exposure}, {cohort}) * {capacity}"
            ),
            outcome=outcome,
            treatments_or_exposures=[exposure, capacity],
            controls=[],
            fixed_effects=[entity, time],
            standard_error_strategy=(
                "cluster_by_policy_assignment_region_with_few_cluster_sensitivity"
            ),
            parameters=design_contract,
            target_claim_ids=claim_ids,
            required_for_admission=True,
    ...
    diagnostics = [
        PlannedStep(
            step_id="diag-cohort-support-and-pretrends",
            name="Cohort support, anticipation and pre-trend diagnostics",
            rationale=(
                "Audit cohort sizes, event-time support, joint pre-trends and anticipation "
                "before admitting any causal claim."
            ),
            required_data_fields=[entity, time, region, cohort, exposure, capacity],
            parameters={**design_contract, "equivalence_rule": "preregister_before_execution"},
            target_claim_ids=claim_ids,
            required_for_admission=True,
            not_executable_reason=not_executable_reason,
        ),
        PlannedStep(
            step_id="diag-capacity-common-support",
            name="Baseline-capacity common-support diagnostics",
            rationale=(
                "Verify overlap by cohort and comparison status; do not extrapolate a sign "
                "switch outside preregistered support."
            ),
            required_data_fields=[cohort, exposure, capacity],
            parameters=strategy_contract,
            not_executable_reason=not_executable_reason,
        ),
        *plan.diagnostics,
    ]
    robustness_tests = [
        PlannedStep(
            step_id="robust-cohort-aggregation",
            name="Cohort and event-time aggregation sensitivity",
            rationale=(
                "Compare prespecified cohort weights without reverting to contaminated "
                "two-way-fixed-effect treatment coefficients."
            ),
            required_data_fields=[cohort, time, exposure, capacity, *outcomes],
            parameters={**design_contract, "aggregation_sensitivity": True},
            not_executable_reason=not_executable_reason,
        ),
        PlannedStep(
            step_id="robust-paired-missingness-denominators",
            name="Paired-outcome missingness and denominator sensitivity",
            rationale=(
                "Report complete-case, selection-weighted and denominator-sensitive results "
                "for disclosure and direct-emissions outcomes."
            ),
            required_data_fields=[entity, time, *outcomes],
            parameters={"missingness": ["complete_case", "selection_weighted"], "paired": True},
            not_executable_reason=not_executable_reason,
        ),
    ...
    falsification_tests = [
        PlannedStep(
            step_id="falsification-no-sign-switch",
            name="Uniform-sign falsification rule",
            rationale=(
                "Falsify the headline hypothesis when the marginal effect keeps one precise "
                "sign over preregistered support or the crossing point is specification-dependent."
            ),
            required_data_fields=[cohort, exposure, capacity, *outcomes],
            parameters={**design_contract, "falsifier": "no_stable_zero_crossing"},
            target_claim_ids=claim_ids,
            required_for_admission=True,
            not_executable_reason=not_executable_reason,
        ),
        PlannedStep(
            step_id="falsification-placebo-cohort-and-spillover",
            name="Placebo timing and spillover falsification",
            rationale=(
                "Use frozen placebo cohorts and neighboring-region exposure definitions to "
                "detect anticipation or contamination."
            ),
            required_data_fields=[region, time, cohort, exposure],
            parameters={"placebo_cohorts": True, "spillover_map_required": True},
            not_executable_reason=not_executable_reason,
        ),
    ...
    mechanism_tests = [
        PlannedStep(
            step_id="mechanism-competing-pathways",
            name="Temporally ordered competing pathways",
            rationale=(
                "Contrast innovation/EPIE pathways by baseline capacity without conditioning "
                "the main causal effect on post-treatment mediators."
            ),
            required_data_fields=[entity, time, cohort, exposure, capacity, *mechanisms],
            parameters={
                "mechanisms": mechanisms,
                "causal_wording": "associational_until_sequential_assumptions_hold",
            },
            not_executable_reason=not_executable_reason,
        ),
    ...
    heterogeneity_tests = [
        PlannedStep(
            step_id="heterogeneity-continuous-capacity",
            name="Continuous pre-policy capacity heterogeneity",
            rationale=(
                "Use the continuous moderator as primary; high/low displays are descriptive "
                "and may not replace the prespecified marginal-effect curve."
            ),
            required_data_fields=[cohort, exposure, capacity, *outcomes],
            parameters={**design_contract, "primary_scale": "continuous"},
            not_executable_reason=not_executable_reason,
        ),
    ...
    return plan.model_copy(
        update={
            ...
            "identification_assumptions": list(
                dict.fromkeys(
                    [
                        "Cohort-specific conditional parallel trends.",
                        "No anticipation before the assigned treatment cohort.",
                        "Auditable, time-consistent firm-region-cohort mapping.",
                        "Strictly pre-treatment moderator measurement.",
                        *plan.identification_assumptions,
                    ]
                )
            ),
            "alternative_explanations": list(
                dict.fromkeys(
                    [
                        ...
                        "Post-treatment changes incorrectly embedded in the capacity measure.",
                        *plan.alternative_explanations,
                    ]
                )
            ),
            "failure_conditions": [
                "No stable sign crossing over preregistered common support.",
                "Cohort/event-time pre-trends or anticipation diagnostics fail.",
                "Firm-region-cohort joins or direct-emissions denominators are not defensible.",
                "Paired greenwashing and direct-emissions results do not align in the high-capacity regime.",
            ],
            "stop_conditions": [
                "Stop after the frozen cohort, event-time, moderator-support and paired-outcome analyses; do not add thresholds or cohorts in response to significance."
            ],
            ...
            "unsupported_requested_analyses": list(
                dict.fromkeys(
                    [
                        not_executable_reason,
                        "The current policy-did-v2 executor is a common-date binary-group DID and is not an implementation of this staggered continuous-capacity DDD contract.",
                        *plan.unsupported_requested_analyses,
                    ]
                )
            ),
            "check_registry_version": _GROUP1_SIGN_SWITCH_REGISTRY_VERSION,
        }
    )
...
class WorkflowEngine:
    ...
    def _normalize_case(self, state: RunState, case: CaseSubmission) -> ResearchPackage:
        ...
        if not case.dataset_refs:
            missing_required_information.append(
                "尚未接入可执行数据资产；本次只能形成研究设计。"
            )
        if case.intake_readiness is not None:
            missing_required_information.extend(
                f"上游可行性阻断：{item}" for item in case.intake_readiness.blockers
            )
            missing_required_information.extend(
                f"上游待核查：{item}" for item in case.intake_readiness.warnings
            )
        package = ResearchPackage(
            **case.model_dump(),
            input_conflicts=[],
    ...
    async def decide_gate(
        ...
        if normalized_gate == "H1" and request.action == "approve":
            readiness = state.case_submission.intake_readiness
            if readiness is not None and not readiness.can_approve_h1:
                summary = "; ".join(readiness.blockers[:3])
                raise WorkflowTransitionError(
                    "H1 approval is blocked by the upstream intake-readiness gate"
                    + (f": {summary}" if summary else "")
                )
        if normalized_gate == "H2" and request.action == "approve":
            readiness = state.case_submission.intake_readiness
            if readiness is not None and not readiness.can_execute:
                summary = "; ".join(readiness.blockers[:3])
                raise WorkflowTransitionError(
                    "H2 execution approval is blocked because executable inputs are not ready; "
                    "use generate_plan_only to freeze the design without statistical execution"
                    + (f": {summary}" if summary else "")
                )
        ...
        try:
            if (
                normalized_gate == "H2"
                and request.action in {"approve", "generate_plan_only"}
            ...
            try:
                if normalized_gate == "H1":
                    ...
                    if request.action not in {"approve", "generate_plan_only"}:
                        raise WorkflowTransitionError(
                            "H2 only accepts approve, generate_plan_only, revise or reject"
                        )
    ...
    async def _after_h1(self, state: RunState) -> None:
        ...
        route = _normalize_group1_sign_switch_route(route, package)
        self._record_step(
            state,
            "method_route",
            "succeeded",
    ...
    async def _generate_design_candidates(
        ...
        *,
        ...
        for strategy, rationale in DESIGN_STRATEGIES:
            ...
            plan = _normalize_group1_sign_switch_candidate_plan(
                plan,
                package,
                strategy,
            )
    ...
    async def _after_h2(
        ...
        *,
        ...
        if common_execution_result is not None:
            ...
            use_fixture = state.execution_mode == "fixture" or state.plan_only
            executor = (
                FixtureExecutor()
                if use_fixture
        ...
        *,
        ...
        if reuse_existing_if_valid and sections:
            try:
                ...
                sections = []
        content_problems = (
            self._manuscript_draft_problems(sections, evidence_pack)
            if sections
            else []
        )
        ...
        if not sections or (not content_problems and not reuse_existing_if_valid):
            ...
            content_problems.extend(
                self._manuscript_draft_problems(
                    sections,
                    evidence_pack,
                )
            )
    ...
    @classmethod
    def _manuscript_draft_problems(
        cls,
        sections: list[ManuscriptSection],
        evidence_pack: dict[str, Any],
    ) -> list[str]:
        problems = cls._manuscript_content_problems(sections, evidence_pack)
        for section in sections:
            content_characters = len(section.content_markdown.strip())
            if content_characters < MIN_FULL_MANUSCRIPT_SECTION_CHARS:
                problems.append(
                    f"{section.section_id} 章节正文少于 H4 最低 "
                    f"{MIN_FULL_MANUSCRIPT_SECTION_CHARS} 字"
                    f"（当前 {content_characters} 字）"
                )
        return problems
