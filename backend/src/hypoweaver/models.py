...
_EXCLUDE_IF_RULES: dict[type[BaseModel], dict[str, Callable[[Any], bool]]] = {}
...
class StrictModel(BaseModel):
    ...
    def __pydantic_init_subclass__(cls, **kwargs: Any) -> None:
        """Backport Pydantic 2.12 ``Field(exclude_if=...)`` to 2.5-2.11."""

        super().__pydantic_init_subclass__(**kwargs)
        rules: dict[str, Callable[[Any], bool]] = {}
        for field_name, field_info in cls.model_fields.items():
            predicate = getattr(field_info, "exclude_if", None)
            extra = field_info.json_schema_extra
            if predicate is None and isinstance(extra, dict):
                candidate = extra.pop("exclude_if", None)
                if callable(candidate):
                    predicate = candidate
            if callable(predicate):
                rules[field_name] = predicate
        _EXCLUDE_IF_RULES[cls] = rules

    @model_serializer(mode="wrap")
    def _serialize_with_exclude_if(self, handler: Any) -> Any:
        serialized = handler(self)
        if not isinstance(serialized, dict):
            return serialized
        for field_name, predicate in _EXCLUDE_IF_RULES.get(type(self), {}).items():
            if field_name in serialized and predicate(getattr(self, field_name)):
...
class UpstreamArtifactRef(StrictModel):
    name: str
    path: str
    sha256: str = Field(pattern=r"^[a-f0-9]{64}$")


class UpstreamProvenance(StrictModel):
    source_system: str
    ...
    artifacts: list[UpstreamArtifactRef] = Field(default_factory=list)
    graph_snapshot_id: str | None = None
    hypothesis_id: str | None = None
    gap_id: str | None = None
    novelty_check_ref: str | None = None
    evidence_refs: list[str] = Field(default_factory=list)
    corpus_boundary: str | None = None


class IntakeReadiness(StrictModel):
    ...
    @model_validator(mode="after")
    def validate_readiness_flags(self) -> "IntakeReadiness":
        if self.status == "blocked" and (self.can_approve_h1 or self.can_execute):
            raise ValueError("blocked intake cannot approve H1 or execute")
        if self.can_execute and not self.can_approve_h1:
            raise ValueError("execution readiness requires H1 approval readiness")
        return self


class Group2DataFeasibilityItem(StrictModel):
    item_id: str
    construct_name: str
    role: str
    definition: str
    ...
    executable_asset_supplied: bool = False
    readiness: Literal["candidate_requires_validation", "missing_blocks_execution"]
    required_join_keys: list[str] = Field(default_factory=list)
    evidence_refs: list[str] = Field(default_factory=list)
    next_action: str


class Group2MethodFeasibilityItem(StrictModel):
    method_id: str
    name: str
    purpose: str
...
class ScientificTenItem(StrictModel):
    item_no: int = Field(ge=1, le=10)
    title: str
    content: str
    status: Literal["evidence_bound", "conditional", "pending"]
    evidence_refs: list[str] = Field(default_factory=list)
    unresolved_actions: list[str] = Field(default_factory=list)


class Group2FeasibilityPackage(StrictModel):
    package_id: str
    upstream_handoff_id: str
    handoff_status: Literal["accepted_for_group2_design", "rejected"]
    execution_status: Literal["ready", "not_ready"]
    go_no_go_decision: Literal[
        "conditional_go_for_design",
        "go_for_execution",
        "no_go_return_to_group1",
    ]
    decision_rationale: str
    return_to_group1_required: bool = False
    data_matrix: list[Group2DataFeasibilityItem] = Field(default_factory=list)
    method_matrix: list[Group2MethodFeasibilityItem] = Field(default_factory=list)
    scientific_ten: list[ScientificTenItem]

    @model_validator(mode="after")
    def validate_scientific_ten(self) -> "Group2FeasibilityPackage":
        item_numbers = [item.item_no for item in self.scientific_ten]
        if item_numbers != list(range(1, 11)):
            raise ValueError("scientific_ten must contain items 1 through 10 in order")
        if self.execution_status == "ready" and any(
            not item.executable_asset_supplied for item in self.data_matrix
        ):
            raise ValueError("execution-ready feasibility requires every data asset")
        return self
...
class CaseSubmission(StrictModel):
    ...
    group2_feasibility_package: Group2FeasibilityPackage | None = None
...
MIN_FULL_MANUSCRIPT_SECTION_CHARS = 180
...
class ManuscriptPackage(StrictModel):
    ...
    def validate_plan_only_boundary(self) -> "ManuscriptPackage":
        ...
        short_sections = [
            ...
            and (
                len(section.content_markdown.strip())
