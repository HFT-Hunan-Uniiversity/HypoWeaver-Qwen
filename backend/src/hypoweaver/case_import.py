...
class DatasetRegistry:
    ...
    def resolve(self, dataset_ref: DatasetRef) -> Path:
        ...
        if not source_path.is_absolute():
            ...
            source_path = (registry_root / source_path).resolve()
            if source_path != registry_root and registry_root not in source_path.parents:
                raise CaseImportError("registered dataset path leaves the registry root")
...
class CaseUploadStore:
    ...
    async def _save(self, filename: str, chunks: AsyncIterable[bytes]) -> Path:
        ...
        upload_count = sum(1 for path in self.root.iterdir() if path.is_dir())
        if upload_count >= MAX_UPLOAD_DIRECTORIES:
