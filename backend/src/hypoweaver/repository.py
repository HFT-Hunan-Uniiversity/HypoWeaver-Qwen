...
from .sqlite_support import connect
...
class RunRepository:
    def __init__(self, path: str | Path | None = None) -> None:
        ...
        product_var_root = (project_root / "backend" / "var").resolve(strict=False)
        self.path = Path(configured) if configured else product_var_root / "hypoweaver.db"
        resolved_parent = self.path.resolve(strict=False).parent
        self.storage_root = (
            product_var_root
            if resolved_parent == product_var_root or product_var_root in resolved_parent.parents
    ...
    def create(self, state: RunState) -> RunState:
        ...
        ensure_local_storage_capacity(
            self.storage_root,
            additional_bytes=len(payload.encode("utf-8")),
        )
        with self._connect() as connection:
            ...
            if run_count >= MAX_LOCAL_RUNS:
                connection.rollback()
                raise LocalStorageLimitError("runs", run_count, MAX_LOCAL_RUNS)
            connection.execute(
