...
def _safe_schema_error_details(
    ...
    *,
    ...
    summary.sort(
        key=lambda issue: (
            any(part.startswith("redacted-") for part in issue.loc),
            issue.loc,
            issue.type,
        )
    )
    return summary[:SCHEMA_ERROR_SUMMARY_LIMIT], len(details)
    ...
    def record_response(
        ...
        *,
        ...
        if provider and model and started_at and completed_at:
            ...
            if not receipt.schema_error_summary:
                receipt_payload.pop("schema_error_summary", None)
            if receipt.schema_error_count == 0:
                receipt_payload.pop("schema_error_count", None)
            with self._lock:
                self.call_receipts.append(receipt_payload)
