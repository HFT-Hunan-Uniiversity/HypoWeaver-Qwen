...
def _is_planned_coefficient_selector(
    *,
    key: str,
    value: Any,
    location: str,
) -> bool:
    if key != "coefficients" or not location.endswith(".parameters"):
        return False
    selectors = [value] if isinstance(value, str) else value
    return (
        isinstance(selectors, list)
        and bool(selectors)
        and all(
            isinstance(item, str) and _SELECTOR.fullmatch(item)
...
def _reject_results(value: Any, location: str) -> None:
    if isinstance(value, Mapping):
        for key, nested in value.items():
            ...
            if normalized in _RESULT_KEYS and not _is_planned_coefficient_selector(
                key=normalized,
                value=nested,
                location=location,
            ):
