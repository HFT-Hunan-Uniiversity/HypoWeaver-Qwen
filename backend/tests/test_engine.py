...
class PersistentShortWriterGateway(FullManuscriptGateway):
    def __init__(self, short_sections: set[str]) -> None:
        self.short_sections = set(short_sections)
        self.calls: list[list[str]] = []
        ...
        return ManuscriptSectionDraftBatch(
            sections=[
                ManuscriptSectionDraft(
                    ...
                    content_template=(
                        "本节仍然过短。"
                        if spec["section_id"] in self.short_sections
                        else self.section_draft(spec).content_template
                    ),
                )
                for spec in payload["section_specs"]
...
class WorkflowEngineTests(unittest.IsolatedAsyncioTestCase):
    ...
    def test_reviewer_critical_issue_uses_code_owned_blocking_rules(
        ...
        policy_registered_diagnostic = human_required.model_copy(
            update={
                "issue_id": "critical-policy-stability",
                "threat_id": "policy.group_stable_entities_only",
            }
        )
        self.assertFalse(
            _is_reviewer_issue_blocking_design(
    ...
    def test_blocked_arena_critic_aggregates_all_candidates(self) -> None:
        ...
        human = technical.model_copy(
            update={
                ...
                "repair_type": "human_required",
                "return_stage": "human",
                "threat_id": "policy.unmapped_human_boundary",
            }
        )
        candidates = [
            DesignCandidate(
    ...
    async def test_short_introduction_and_conclusion_repair_then_fallback(
        self,
    ) -> None:
        run = await self._retryable_research_writer_run()
        target_ids = {"introduction", "conclusion"}
        gateway = PersistentShortWriterGateway(target_ids)
        ...
        self.assertEqual((run.status, run.current_gate), ("waiting_human", "H4"))
        requested_ids = [
            section_id for call in gateway.calls for section_id in call
        ]
        for section_id in FULL_MANUSCRIPT_SECTION_IDS:
            self.assertEqual(
                requested_ids.count(section_id),
        ...
        for section_id in target_ids:
            ...
            self.assertGreaterEqual(len(section.content_markdown.strip()), 180)
            self.assertTrue(
                section.content_markdown.startswith(
                    DETERMINISTIC_SAFE_SECTION_TEXTS[section_id][:80]
                )
            )
        fallback_steps = [
        ...
        for step in fallback_steps:
            self.assertTrue(
                any(
                    "当前" in problem and "180" in problem
                    for problem in step.input["source_problems"]
                )
            )
