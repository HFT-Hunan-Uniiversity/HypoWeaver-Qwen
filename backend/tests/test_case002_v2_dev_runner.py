...
def _build_case002_fixture(
    ...
    (adapter_root / "__init__.py").write_text("\n", encoding="utf-8")
    (adapter_root / "cli.py").write_text("# fixture adapter\n", encoding="utf-8")
    for filename in ("agents.py", "ai_lab_repo.py", "mlesolver.py", "papersolver.py"):
        (agent_root / filename).write_text(
            f"# fixture upstream source: {filename}\n",
            encoding="utf-8",
        )

    shared_visible = {
        "main_data.csv": "firm,year,treat,outcome\nA,2020,0,1\n",
        "data_dictionary.csv": "name,role\ntreat,exposure\noutcome,outcome\n",
        "data_description.md": "Frozen Case002 fixture data.\n",
    }
    for view, directory_name in CASE_DIRECTORIES.items():
        case_root = workspace_root / "benchmark-cases" / directory_name
        visible_root = case_root / "01_model_input"
        visible_root.mkdir(parents=True)
        for filename, content in shared_visible.items():
            (visible_root / filename).write_text(content, encoding="utf-8")
        policy = {
        ...
        (visible_root / "case_profile.json").write_text(
            json.dumps(
                {
                    "case_id": INDEPENDENT_CASE_ID,
                    "policy_design": policy,
                }
            ),
            encoding="utf-8",
        )
        ...
        manifest = {
            ...
            "policy_design_freeze": policy,
            "visible_input": {"files": visible_files},
        }
...
class Case002V2DevRunnerTests(unittest.TestCase):
    ...
    def test_preflight_hashes_public_model_disclosure(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            _build_case002_fixture(workspace_root, runtime)
            with (
                patch(
    ...
    def test_case002_given_input_track_has_no_literature_completion_gate(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            _build_case002_fixture(workspace_root, _runtime())
            for view, directory_name in CASE_DIRECTORIES.items():
                case_root = workspace_root / "benchmark-cases" / directory_name
                config = json.loads(
                ...
                self.assertEqual(
                    config["workflow"]["benchmark_task_track"],
                    BENCHMARK_TASK_TRACK,
                )
                self.assertEqual(config["workflow"]["max_steps"], 5)
                self.assertEqual(config["workflow"]["num_papers_lit_review"], 0)
                self.assertEqual(config["workflow"]["mlesolver_max_steps"], 1)
                self.assertEqual(config["workflow"]["papersolver_max_steps"], 0)
                self.assertEqual(
                    manifest["benchmark_task_track"], BENCHMARK_TASK_TRACK
                )
                self.assertEqual(
                    manifest["agent_laboratory_budget"], expected_budget
                )
                policy = json.loads(
