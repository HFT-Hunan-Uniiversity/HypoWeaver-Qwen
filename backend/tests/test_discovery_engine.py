...
from hypoweaver.discovery_engine.builder import (
    DiscoveryBuildError,
    build_discovery_outputs,
    write_discovery_release,
)
from hypoweaver.discovery_engine.validate import (
...
def base_graph() -> dict:
    graph = json.loads(
        (ROOT / "backend" / "config" / "discovery_schemas" / "research_graph.seed.json").read_text(encoding="utf-8")
    )
...
class DiscoveryBuilderTests(unittest.TestCase):
    def setUp(self) -> None:
        self.test_root = ROOT / ".test-tmp" / uuid4().hex
    ...
    def test_build_and_release_bytes_are_deterministic(self) -> None:
        ...
        one = self.test_root / "discovery_release_test_one"
        two = self.test_root / "discovery_release_test_two"
