...
from hypoweaver.case_import import (
    CaseImportError,
    CaseUploadStore,
    DatasetRegistry,
    LocalCaseImporter,
)
from hypoweaver.models import DatasetRef
...
class LocalCaseImporterTests(unittest.TestCase):
    ...
    def test_import_infers_case_fields_hash_and_private_registry(self) -> None:
        ...
        expected_private_mode = 0o666 if os.name == "nt" else 0o600
        self.assertEqual(
            stat.S_IMODE(self.registry_path.stat().st_mode),
            expected_private_mode,
        )
    ...
    def test_registry_resolves_portable_relative_path_and_rejects_escape(self) -> None:
        registry_root = Path(self.tempdir.name) / "portable"
        dataset_path = registry_root / "data" / "main.csv"
        ...
        portable_registry = DatasetRegistry(registry_root / "datasets.json")
        portable_registry.path.write_text(
            json.dumps(
                {
                    dataset_ref.dataset_id: {
                        "source_path": "data/main.csv",
                        "sha256": digest,
                        "filename": "main.csv",
                        "size_bytes": dataset_path.stat().st_size,
                    }
        ...
        self.assertEqual(portable_registry.resolve(dataset_ref), dataset_path.resolve())

        payload = json.loads(portable_registry.path.read_text(encoding="utf-8"))
        payload[dataset_ref.dataset_id]["source_path"] = "../outside.csv"
        portable_registry.path.write_text(json.dumps(payload), encoding="utf-8")
        with self.assertRaisesRegex(CaseImportError, "leaves the registry root"):
            portable_registry.resolve(dataset_ref)
