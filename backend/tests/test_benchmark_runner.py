...
class AgentLaboratoryRunnerTests(unittest.TestCase):
    ...
    def test_prepare_case_keeps_only_visible_input_and_same_dataset_hash(self) -> None:
        ...
        expected_read_only_mode = 0o444 if os.name == "nt" else 0o400
        self.assertEqual(
            (visible / "main_data.csv").stat().st_mode & 0o777,
            expected_read_only_mode,
        )
    ...
    def test_adapter_process_runs_from_isolated_workspace(self) -> None:
        ...
        self.assertEqual(captured["command"][0], str(SANDBOX_EXEC))
        profile = (workspace / "agent-lab.sb").read_text(encoding="utf-8")
        self.assertIn("deny file-read", profile)
        self.assertIn("(deny file-write*)", profile)
        self.assertIn(
            f'(allow file-write* (subpath "{_sandbox_path(workspace / "output")}"))',
            profile,
        )
        self.assertIn(_sandbox_path(self.runner.agent_lab_root), profile)
        self.assertTrue(
    ...
    def test_explicit_hidden_reference_path_is_denied_to_generated_code(self) -> None:
        ...
        profile = (workspace / "agent-lab.sb").read_text(encoding="utf-8")
        self.assertIn(
            f'(deny file-read* (subpath "{_sandbox_path(hidden_reference)}"))',
            profile,
        )
