...
class CommonExecutorAdapterTests(unittest.IsolatedAsyncioTestCase):
    ...
    def test_h2_boundary_allows_only_named_coefficient_selectors(self) -> None:
        planned = self.run.model_copy(deep=True)
        arena = copy.deepcopy(planned.artifacts["design_arena"]["payload"])
        arena["selector_fixture"] = {
            "parameters": {"coefficients": "pre_policy_leads"}
        }
        self.engine._put_artifact(planned, "design_arena", arena)
        assert_h2_pre_result_boundary(planned)

        leaked = self.run.model_copy(deep=True)
        arena = copy.deepcopy(leaked.artifacts["design_arena"]["payload"])
        arena["selector_fixture"] = {
            "parameters": {"coefficients": [0.25]}
        }
        self.engine._put_artifact(leaked, "design_arena", arena)
        with self.assertRaisesRegex(
            CommonExecutorAdapterError,
            "post-result field is forbidden",
        ):
            assert_h2_pre_result_boundary(leaked)

        misplaced = self.run.model_copy(deep=True)
        arena = copy.deepcopy(misplaced.artifacts["design_arena"]["payload"])
        arena["coefficients"] = "pre_policy_leads"
        self.engine._put_artifact(misplaced, "design_arena", arena)
        with self.assertRaisesRegex(
            CommonExecutorAdapterError,
            "post-result field is forbidden",
        ):
            assert_h2_pre_result_boundary(misplaced)
