...
- React + Vite 研究工作台；
- 研究资源库把“系统论文库”和“我的原始 PDF”分开呈现：系统库支持 BGE + 图谱检索和单篇正文阅读，上传区用于逐页阅读原始论文；
- 原始 PDF 文献阅读工作台，保留文件/页级哈希，按页显示原文，支持限定整篇或当前页的 Qwen 问答、原始页码引用和明确的本机删除操作；
- FastAPI 工作流 API 与自动 OpenAPI 文档；
- H1 研究边界、H2 设计冻结、H3 逐条主张授权、H4 封存；
- Fixture、Qwen 和 `code_owned` 三种模型路由；
- 独立只读 Knowledge Service，兼容 8.23 分支的 BGE/HDF5 向量与 JSON 知识图谱资产；
- 证据约束的 Qwen DiscoveryPlan、H0 人工审阅，以及确定性的 GapCard/HypothesisCard 生成；
- 独立 Research Engine，支持真实面板、政策因果、空间模型与独立复现；
- SQLite 状态仓库、不可变数据引用、哈希绑定和 HMAC-SHA256 封存；
- Group1 → Group2 冻结交接包接入，以及在线 Discovery Engine → H1 交接；
- Docker Compose 三服务部署和 GitHub Actions 回归。
...
当前本地 8.23 快照包含 1,169 条论文记录：455 条在注册表中标为全文，但其中只有 105 条清洗文本达到可通读正文标准，另 350 条只是很短的索引壳；其余 714 条只有元数据。它不是原始 PDF 馆藏，也没有随交付提供在线爬虫。资源库会优先展示这 105 篇可阅读正文，并把其余记录标成索引或题录。
...
要使用真实发现链路，还需设置 `DASHSCOPE_API_KEY`，并把 8.23 知识资产以只读方式放入 `knowledge-data/`。目录布局和 BGE 模型兼容要求见[知识发现整合指南](docs/KNOWLEDGE_DISCOVERY_INTEGRATION.md)。
...
容器把工作流、Knowledge Service 和 Research Engine 作为三个隔离服务运行。Knowledge Service 只读挂载知识资产；工作流与 Research Engine 共享私有运行数据卷。只有工作流端口 `8000` 对宿主机开放。
...
终端 1：Knowledge Service（示例使用 8.23 只读资产）。

```bash
# Windows PowerShell
$env:HYPOWEAVER_KNOWLEDGE_VECTOR_PATH='D:\knowledge-data\all_store.h5'
$env:HYPOWEAVER_KNOWLEDGE_CLEANED_DIR='D:\knowledge-data\cleaned'
$env:HYPOWEAVER_KNOWLEDGE_METADATA_DIR='D:\knowledge-data\cleaned_meta'
$env:HYPOWEAVER_KNOWLEDGE_GRAPH_DIR='D:\knowledge-data\kg'
$env:HYPOWEAVER_KNOWLEDGE_EMBEDDING_BACKEND='auto'
$env:KNOWLEDGE_SERVICE_TOKEN='replace-with-local-token'
python -m uvicorn hypoweaver.knowledge_api:app --app-dir backend/src --host 127.0.0.1 --port 8002
```

`auto` 在发现 HDF5 时使用与 8.23 分支一致的 BGE 预处理。若当时建库实际发生了规则向量降级，必须改为 `legacy_rule`；不要混用向量空间。

终端 2：Research Engine。
...
$env:KNOWLEDGE_SERVICE_URL='http://127.0.0.1:8002'
$env:KNOWLEDGE_SERVICE_TOKEN='replace-with-local-token'
...
KNOWLEDGE_SERVICE_URL=http://127.0.0.1:8002 \
KNOWLEDGE_SERVICE_TOKEN=replace-with-local-token \
python -m uvicorn hypoweaver.api:app --app-dir backend/src --host 127.0.0.1 --port 8000
```
...
- [知识发现整合指南](docs/KNOWLEDGE_DISCOVERY_INTEGRATION.md)
...
```text
backend/
  src/hypoweaver/       工作流、知识检索、Discovery Engine、统计执行、复现、图表与 API
  tests/                 后端回归套件
...
Dockerfile.knowledge     Knowledge Service 专用镜像
docker-compose.yml       三服务、只读知识资产与持久卷编排
```
...
本次本地整合验收：安装 Knowledge Service 依赖后，后端 613 项全部通过（包含 HDF5 实读、权威全文边界、单篇正文阅读、论文目录和真实检索）；前端 64 项通过，生产构建成功。未安装 `h5py` 的轻量 CI 会明确跳过 1 项 HDF5 实读测试。GitHub Actions 会在每次 push 和 pull request 上重复执行轻量环境检查。
