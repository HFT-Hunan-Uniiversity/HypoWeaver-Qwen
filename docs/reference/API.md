...
curl "$HW_BASE_URL/api/v1/knowledge/health"
...
## 6. 在线研究发现：EvidenceBundle → H0 → H1

生成一个受证据约束的候选计划：

```bash
curl -X POST "$HW_BASE_URL/api/v1/discovery/plans/generate" \
  -H "Content-Type: application/json" \
  -H "X-Hypoweaver-Token: $HW_TOKEN" \
  -d '{
    "search": {
      "question": "绿色金融政策如何影响企业绿色创新？",
      "top_k": 20,
      "max_graph_edges": 30
    },
    "context": {
      "goal": "形成一个可证伪、可进入 H1 设计审阅的候选假设",
      "unit_of_analysis": "企业—年度",
      "sample_period": "2012—2024",
      "constraints": ["研究空白只限当前语料"]
    }
  }'
```

响应包含完整 `evidence_bundle` 和 `plan`。客户端应把二者原样保留在当前审阅会话中，不要只保留模型摘要。

H0 审阅预览：

```http
POST /api/v1/discovery/plans/review
Content-Type: application/json
X-Hypoweaver-Token: <workflow-token>

{
  "evidence_bundle": { "...": "generate 响应中的完整对象" },
  "plan": { "...": "generate 响应中的完整对象" },
  "review_note": "已逐条核对 chunk 引用、语料边界、机制链与证伪条件。"
}
```

该调用会执行 Group1 schema 与语义校验，但不创建 run。批准进入 H1：

```http
POST /api/v1/discovery/plans/launch
Content-Type: application/json
X-Hypoweaver-Token: <workflow-token>

{
  "evidence_bundle": { "...": "完整对象" },
  "plan": { "...": "完整对象" },
  "review_note": "已逐条核对 chunk 引用、语料边界、机制链与证伪条件。",
  "approve_h0": true,
  "approval_reason": "该候选足以进入 H1，但仍需绑定真实数据并审阅识别条件。",
  "mode": "research",
  "research_model_provider": "qwen"
}
```

成功响应中的 `run` 停在 H1。没有真实 `dataset_refs` 时，`intake_readiness.can_execute=false`，后续不能越过执行前置条件。图谱关系候选不会自动变成科学证据。
...
| GET | `/api/v1/knowledge/health` | 读取内部知识服务状态 |
| POST | `/api/v1/knowledge/search` | 获取可追溯 EvidenceBundle |
| POST | `/api/v1/discovery/plans/generate` | 检索证据并生成 Qwen DiscoveryPlan |
| POST | `/api/v1/discovery/plans/review` | H0 审阅并生成 Group1 发布预览 |
| POST | `/api/v1/discovery/plans/launch` | H0 批准并创建停在 H1 的 run |
...
| GET/POST | `/api/v1/literature/documents` | 列出/上传原始 PDF（POST body 为 `application/pdf`） |
| GET | `/api/v1/literature/documents/{document_id}` | 读取原始 PDF 清单与文件/页级哈希 |
| GET | `/api/v1/literature/documents/{document_id}/file` | 下载经哈希校验的原始 PDF |
| GET | `/api/v1/literature/documents/{document_id}/pages/{page_number}` | 读取指定页的原始文本层 |
| GET | `/api/v1/literature/documents/{document_id}/pages/{page_number}/image` | 渲染指定原始 PDF 页面 |
| POST | `/api/v1/literature/documents/{document_id}/ask` | 仅基于服务端原文页调用 Qwen，并返回原始页码引用 |
...
| 503 | 知识服务、模型或执行器未配置/不可用 | 检查 knowledge health、runtime config 与服务日志 |
