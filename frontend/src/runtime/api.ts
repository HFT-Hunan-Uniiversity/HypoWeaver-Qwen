...
export function normalizeRun(payload: unknown): RunSnapshot {
  ...
  const caseSubmission = asRecord(first(run, 'case_submission', 'caseSubmission'))
  const upstreamProvenance = asRecord(first(caseSubmission, 'upstream_provenance', 'upstreamProvenance'))
  const intakeReadiness = asRecord(first(caseSubmission, 'intake_readiness', 'intakeReadiness'))
  const group2Feasibility = asRecord(first(caseSubmission, 'group2_feasibility_package', 'group2FeasibilityPackage'))
  ...
  return {
    ...
    upstreamPackage: Object.keys(upstreamProvenance).length ? {
      ...
      packageId: asString(first(upstreamProvenance, 'package_id', 'packageId')),
    ...
    intakeReadiness: Object.keys(intakeReadiness).length ? {
      status: asString(intakeReadiness.status, 'blocked') as 'ready' | 'conditional' | 'blocked',
      canApproveH1: Boolean(first(intakeReadiness, 'can_approve_h1', 'canApproveH1')),
      canExecute: Boolean(first(intakeReadiness, 'can_execute', 'canExecute')),
      blockers: asArray(intakeReadiness.blockers).map(String),
      warnings: asArray(intakeReadiness.warnings).map(String),
      requiredInputs: asArray(first(intakeReadiness, 'required_inputs', 'requiredInputs')).map(String),
      methodRequirements: asArray(first(intakeReadiness, 'method_requirements', 'methodRequirements')).map(String),
    } : undefined,
    group2Feasibility: Object.keys(group2Feasibility).length ? {
      packageId: asString(first(group2Feasibility, 'package_id', 'packageId')),
      handoffStatus: asString(first(group2Feasibility, 'handoff_status', 'handoffStatus'), 'rejected') as 'accepted_for_group2_design' | 'rejected',
      executionStatus: asString(first(group2Feasibility, 'execution_status', 'executionStatus'), 'not_ready') as 'ready' | 'not_ready',
      goNoGoDecision: asString(first(group2Feasibility, 'go_no_go_decision', 'goNoGoDecision'), 'no_go_return_to_group1') as 'conditional_go_for_design' | 'go_for_execution' | 'no_go_return_to_group1',
      decisionRationale: asString(first(group2Feasibility, 'decision_rationale', 'decisionRationale')),
      returnToGroup1Required: Boolean(first(group2Feasibility, 'return_to_group1_required', 'returnToGroup1Required')),
      dataMatrix: asArray(first(group2Feasibility, 'data_matrix', 'dataMatrix')).map((value) => {
        const item = asRecord(value)
        return {
          itemId: asString(first(item, 'item_id', 'itemId')),
          constructName: asString(first(item, 'construct_name', 'constructName', 'construct')),
          role: asString(item.role),
          candidateSourceIds: asArray(first(item, 'candidate_source_ids', 'candidateSourceIds')).map(String),
          sourceAccess: asString(first(item, 'source_access', 'sourceAccess')),
          executableAssetSupplied: Boolean(first(item, 'executable_asset_supplied', 'executableAssetSupplied')),
          readiness: asString(item.readiness),
          nextAction: asString(first(item, 'next_action', 'nextAction')),
        }
      }),
      methodMatrix: asArray(first(group2Feasibility, 'method_matrix', 'methodMatrix')).map((value) => {
        const item = asRecord(value)
        return {
          methodId: asString(first(item, 'method_id', 'methodId')),
          name: asString(item.name),
          purpose: asString(item.purpose),
          implementationStatus: asString(first(item, 'implementation_status', 'implementationStatus')),
          nextAction: asString(first(item, 'next_action', 'nextAction')),
        }
      }),
      scientificTen: asArray(first(group2Feasibility, 'scientific_ten', 'scientificTen')).map((value) => {
        const item = asRecord(value)
        return {
          itemNo: Number(first(item, 'item_no', 'itemNo') ?? 0),
          title: asString(item.title),
          content: asString(item.content),
          status: asString(item.status, 'pending') as 'evidence_bound' | 'conditional' | 'pending',
          evidenceRefs: asArray(first(item, 'evidence_refs', 'evidenceRefs')).map(String),
          unresolvedActions: asArray(first(item, 'unresolved_actions', 'unresolvedActions')).map(String),
        }
      }),
    } : undefined,
...
export const workflowApi = {
  ...
  async startGroup1Handoff(path: string, mode: 'research' | 'fixture' = 'research'): Promise<RunSnapshot> {
    if (!path.trim()) throw new Error('请先填写 Group 1 冻结交接包路径。')
